...
//--------------------------------------------------------------------------------------------------전화번호 숫자만 입력하게   
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == loginBtn) {// 확인 버튼 누르면
			...
			if (result == JOptionPane.YES_OPTION) {// yes버튼을 누르면
				String tel = cafeDTO.getTel(); // 원본전화번호
				updateCafe(tel);
   ...
   private void updateCafe(String tel) {
      ...
      int su = cafeDAO.updateCafe(cafeDTO, tel); //변경된 cafeDTO와 원본tel 준다
      if(su==1) {
    	  JOptionPane.showMessageDialog(this, "수정 완료 했습니다.");
    	  setVisible(false);
      }else {
    	  JOptionPane.showMessageDialog(this, "수정 실패 했습니다.");
    	  nameT.setText("");
          phoneComboBox.setSelectedItem("010");
          tel2T.setText("");
          tel3T.setText("");
